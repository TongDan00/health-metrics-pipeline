...
A data pipeline and analysis project tracking daily type 1 diabetic health metrics, including diet and blood sugar, using Python, SQL and Tableau.

## Medical Context: Understanding the Data
*For reviewers without a medical background:* Managing Type 1 Diabetes (T1D) is essentially a 24/7 data analysis job. Unlike Type 2 diabetes, T1D means the pancreas produces zero insulin. To maintain stable blood glucose levels, a patient must constantly balance their carbohydrate intake against calculated **"bolus"** (mealtime) insulin doses. 

This is highly complex because it isn't just about food. Variables like sleep, physical activity intensity, stress, and protein digestion curves all drastically alter how the body reacts to insulin. This project models and analyzes that complex physiological web.
...
## Future Roadmap & Scaling: Advanced Biological Logic
To make this pipeline even more robust, I am actively building out the following features:

- *Scaling to Cohort Analysis (Big Data)*: Modifying the Python generator to simulate data for hundreds of unique patients (adding user_id relationships). This will allow for macro-trend analysis and demonstrate the pipeline's capability to handle large-scale, enterprise-level relational databases.

- *Macronutrient Tracking & The Protein Curve*: Adding precise calorie and portion metrics. Accounting for protein's delayed impact on blood sugar, which typically peaks 1 to 2 hours post-ingestion and affects levels for up to 5 hours.

- *External Factors & Illness*: Adding logic to handle everyday variables like caffeine (moderate insulin resistance) and different artificial sweeteners. For illness, the model focuses on decreased insulin sensitivity and higher glucose as the main drivers, but it also accounts for sudden CGM drops and electrolyte loss from stomach issues.
